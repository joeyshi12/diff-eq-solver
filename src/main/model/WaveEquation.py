...
class WaveEquation(PDE):
    ...
    def integrate_neumann(self, dx: float, dt: float, n: int, m: int, k: float) -> np.array:
        ...
        u_1 = [u_0[0] - 2 * self.p(dt) * dx] + u_1 + [u_0[-1] + 2 * self.q(dt) * dx]    # next boundary conditions
        u.append(u_1)
        for j in range(2, m + 1):
    ...
    def integrate_mixed_1(self, dx: float, dt: float, n: int, m: int, k: float) -> np.array:
        ...
        u_0 = [self.p(0)] + u_0 + [u_0[-1] + 2 * self.q(0) * dx]            # initial boundary conditions
        u.append(u_0)
        u_1 = [(k ** 2 * u_0[i + 1] + 2 * (1 - k ** 2) * u_0[i] + k ** 2 * u_0[i - 1]) / 2 + dt * self.g(i * dx)
               for i in range(1, n + 1)]                                    # next values
        u_1 = [self.p(dt)] + u_1 + [u_0[-1] + 2 * self.q(dt) * dx]          # next boundary conditions
        u.append(u_1)
        for j in range(2, m + 1):
    ...
    def integrate_mixed_2(self, dx: float, dt: float, n: int, m: int, k: float) -> np.array:
        ...
        u_0 = [u_0[0] - 2 * self.p(0) * dx] + u_0 + [self.q(0)]             # initial boundary conditions
        u.append(u_0)
        u_1 = [(k ** 2 * u_0[i + 1] + 2 * (1 - k ** 2) * u_0[i] + k ** 2 * u_0[i - 1]) / 2 + dt * self.g(i * dx)
               for i in range(n)]                                           # next values
        u_1 = [u_0[0] - 2 * self.p(dt) * dx] + u_1 + [self.q(dt)]           # next boundary conditions
        u.append(u_1)
        for j in range(2, m + 1):
            u_j = [self.node_val(u, k, i, j) for i in range(1, n + 1)]      # nodal values
            u_j = [u_j[1] - 2 * self.p(j * dt)] + u_j + [self.q(j * dt)]    # boundary values
            u.append(u_j)
        print(u)
        u = np.delete(u, 0, 1)
        return np.array(u)

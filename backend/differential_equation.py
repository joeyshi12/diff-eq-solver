...
class ODE(DifferentialEquation, ABC):
    ...
    def display(self):
        ...
        plt.plot(np.arange(N) * self.dt, self.solution)
        plt.xlabel("t [time]")
        plt.ylabel("x(t)")
    ...
    def record_solution(self, path):
        ...
        workbook = xlsxwriter.Workbook(os.path.join(dirname, path))
...
class TimeDependent1D(DifferentialEquation, ABC):
    ...
    def display(self):
        ...
        line, = ax.plot(x, self.solution[0], "-o", markersize=4)
        ...
        ani = animation.FuncAnimation(fig, update_plot, frames=K, blit=True, interval=20)
        plt.xlim(0, (N - 1) * self.dx)
        plt.ylim(np.min(self.solution), np.max(self.solution))
        plt.xlabel("x [length]")
        plt.ylabel("u(x, t)")
        plt.title("Solution: u(x, t)")
        plt.show()

    def display_3d_plot(self):
        K, N = self.solution.shape
